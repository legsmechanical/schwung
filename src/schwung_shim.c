...
static void shim_post_transfer(void *ctx, uint8_t *shadow, const uint8_t *hw, int size)
{
    ...
    if (shadow_display_mode && shadow_control) {
         ...
         *
         * #11 davebox co-run knob-desync fix (A): the synthetic CC occupies a
         * MIDI_IN slot, and shadow_drain_midi_inject defers note delivery to Move
         * tracks whenever ANY MIDI_IN slot is non-zero (SIGABRT-avoidance guard,
         ...
         * sequenced notes to Move-native instruments (db "drops out, catches up
         * at a different place"; measured inj_starved ~235/s). Fix: fold detents
         * into a PERSISTENT accumulator and emit only 1 frame in KNOB_EMIT_PERIOD,
