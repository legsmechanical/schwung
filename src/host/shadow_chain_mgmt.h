...
/* Check if any send FX slot on a bus has a loaded instance.
 * Intentionally ignores per-slot `bypassed`: a fully-bypassed bus is treated as
 * "active" so the dry accumulated send still returns at return_level (bypass ==
 * passthrough of the send signal, not a return mute). To silence a bus's return
 * entirely, set its return_level to 0. */
